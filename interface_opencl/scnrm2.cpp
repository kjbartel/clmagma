...
magmablas_scnrm2_adjust(int k, magmaDouble_ptr xnorm, size_t xnorm_offset, magmaFloatComplex_ptr c, size_t c_offset, magma_queue_t queue)
{
    ...
    ckKernel = rt->KernelPool["magmablas_scnrm2_adjust_kernel"];
    if (!ckKernel)
    {
...
*/
extern "C" magma_err_t
magmablas_scnrm2(int m, int num, magmaFloatComplex_ptr da, size_t da_offset, magma_int_t ldda, 
                 magmaDouble_ptr dxnorm, size_t dxnorm_offset, magma_queue_t queue) 
{
    ...
    ckKernel = rt->KernelPool["magmablas_scnrm2_kernel"];
    if (!ckKernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
