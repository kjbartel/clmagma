        ...
        magma_queue_t queue )
{
    ...
    {
        zlaswp_params_t2 params = { n, lda, ind + k, BLOCK_SIZE };
    ...
    zlaswp_params_t2 params = { n, lda, ind + k, num_pivots };
...
// ----------------------------------------
// offset is already added to ipiv, used in zgetrf2_mgpu
magma_err_t
magma_zpermute_long3(int n,
        cl_mem dAT, size_t dAT_offset, int lda,
        int *ipiv, int nb, int ind,
        magma_queue_t queue )
{
    for( int k = 0; k < nb; k += BLOCK_SIZE ) {
        ...
        zlaswp_params_t2 params = { n, lda, ind + k, npivots };
        for( int j = 0; j < BLOCK_SIZE; ++j ) {
