...
typedef struct {
/*
    magmaFloatComplex_ptr A1;
    magmaFloatComplex_ptr A2;
*/
...
extern "C" void
magmablas_cswap( magma_int_t n, magmaFloatComplex_ptr dA1T, size_t offset_dA1T, magma_int_t lda1,
                 magmaFloatComplex_ptr dA2T, size_t offset_dA2T, magma_int_t lda2, magma_queue_t queue)
{
    int  blocksize = BLOCK_SIZE;
    size_t LocalWorkSize[1] = {blocksize};
    size_t GlobalWorkSize[1] = {((n+blocksize-1)/blocksize)*LocalWorkSize[0]};

    magmagpu_cswap_params_t params = { n, (int)offset_dA1T, lda1, (int)offset_dA2T, lda2 };
    
    cl_int ciErrNum;                // Error code var
    cl_kernel ckKernel=NULL;
    
    ckKernel = rt->KernelPool["magmagpu_cswap"];
    if (!ckKernel)
    {
    ...
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmagpu_cswap_params_t),           (void*)&params    );
...
extern "C" void
magmablas_cempty( magma_queue_t queue, magmaDouble_ptr dA, magmaDouble_ptr dB, magmaDouble_ptr dC)
{
    int m = 1, n =1;
    ...
    ckKernel = rt->KernelPool["cswap_empty_kernel"];
    if (!ckKernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return;
