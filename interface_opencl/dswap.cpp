...
extern "C" void
magmablas_dswap( magma_int_t n, magmaDouble_ptr dA1T, size_t offset_dA1T, magma_int_t lda1,
                 magmaDouble_ptr dA2T, size_t offset_dA2T, magma_int_t lda2, magma_queue_t queue)
{
    ...
    magmagpu_dswap_params_t params = { n, (int)offset_dA1T, lda1, (int)offset_dA2T, lda2 };
    
    cl_int ciErrNum;                // Error code var
    cl_kernel ckKernel=NULL;
    
    ckKernel = rt->KernelPool["magmagpu_dswap"];
    if (!ckKernel)
    {
    ...
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmagpu_dswap_params_t),           (void*)&params    );
...
magmablas_dempty( magma_queue_t queue, magmaDouble_ptr dA, magmaDouble_ptr dB, magmaDouble_ptr dC)
{
    ...
    ckKernel = rt->KernelPool["dswap_empty_kernel"];
    if (!ckKernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return;
