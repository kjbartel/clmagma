 ...
 *      May 2012
 *
 * @generated c Wed Oct 24 00:32:57 2012
 */
...
void czero_nbxnb_block(int nb, cl_mem dA, size_t dA_offset, int ldda, magma_queue_t queue)
{
	cl_int ciErrNum;
	cl_kernel ckKernel=NULL;

	ckKernel = rt->KernelPool["cset_nbxnb_to_zero"];
	if(!ckKernel)
	{
...
*/
extern "C" void 
magmablas_claset(int uplo, magma_int_t m, magma_int_t n, cl_mem A, size_t A_offset, magma_int_t lda, magma_queue_t queue)
{
	size_t LocalWorkSize[2] = {claset_threads, 1};
	size_t GlobalWorkSize[2] = {(m/claset_threads+(m % claset_threads != 0))*LocalWorkSize[0], 
	...
	if(m!=0 && n!=0){
		if (uplo == MagmaLower){
			ckKernel = rt->KernelPool["claset_lower"];
			if(!ckKernel){
				printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
				return;
			}
			...
			ckKernel = rt->KernelPool["claset_upper"];
			if(!ckKernel){
				printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
				return;
			}
