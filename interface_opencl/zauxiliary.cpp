...
void zzero_nbxnb_block(int nb, cl_mem dA, size_t dA_offset, int ldda, magma_queue_t queue)
{
	cl_int ciErrNum;
	cl_kernel ckKernel=NULL;

	ckKernel = rt->KernelPool["zset_nbxnb_to_zero"];
	if(!ckKernel)
	{
...
*/
extern "C" void 
magmablas_zlaset(int uplo, magma_int_t m, magma_int_t n, cl_mem A, size_t A_offset, magma_int_t lda, magma_queue_t queue)
{
	size_t LocalWorkSize[2] = {zlaset_threads, 1};
	size_t GlobalWorkSize[2] = {(m/zlaset_threads+(m % zlaset_threads != 0))*LocalWorkSize[0], 
	...
	if(m!=0 && n!=0){
		if (uplo == MagmaLower){
			ckKernel = rt->KernelPool["zlaset_lower"];
			if(!ckKernel){
				printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
				return;
			}
			...
			ckKernel = rt->KernelPool["zlaset_upper"];
			if(!ckKernel){
				printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
				return;
			}
