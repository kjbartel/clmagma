...
static int diffMatrix( magmaFloatComplex *A, magmaFloatComplex *B, int m, int n, int lda){
    int i, j;
    for(i=0; i<m; i++) {
        for(j=0; j<n; j++)
            if ( !(MAGMA_C_EQUAL( A[lda*j+i], B[lda*j+i] )) ){
...
int main( int argc, char** argv) 
{
	...
	TESTING_MALLOC(h_A1, magmaFloatComplex, n2);
	TESTING_MALLOC(h_A2, magmaFloatComplex, n2);
	TESTING_MALLOC(h_A3, magmaFloatComplex, n2);
	TESTING_MALLOC(h_AT, magmaFloatComplex, n2);
