    ...
    for( int iuplo = 0; iuplo < 3; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_zsetmatrix( M, N, h_B, ldb, d_B, 0, lddb, opts.queue );
            
            gpu_time = magma_sync_wtime( 0 );
            //magmablas_zlacpy( uplo[iuplo], M-2, N-2, d_A+1+ldda, 0, ldda, d_B+1+lddb, 0, lddb, opts.queue );  // inset by 1 row & col
            magmablas_zlacpy( uplo[iuplo], M, N, d_A, 0, ldda, d_B, 0, lddb, opts.queue );
            ...
            if ( opts.verbose ) {
                ...
                printf( "dA=" );  magma_zprint_gpu( M, N, d_A, 0, ldda, opts.queue );
                printf( "dB=" );  magma_zprint_gpu( M, N, d_B, 0, ldda, opts.queue );
            }
            
            /* =====================================================================
               Check the result
               =================================================================== */
            magma_zgetmatrix( M, N, d_B, 0, ldda, h_R, lda, opts.queue );
