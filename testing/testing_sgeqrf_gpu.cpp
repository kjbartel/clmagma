/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from testing_zgeqrf_gpu.cpp normal z -> s, Sat Nov 15 00:21:40 2014
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_ssetmatrix( M, N, h_R, lda, d_A, 0, ldda, opts.queue );
            ...
            if ( opts.version == 2 ) {
                magma_sgeqrf2_gpu( M, N, d_A, 0, ldda, tau, opts.queues2, &info );
            }
            else {
                ...
                if ( opts.version == 1 ) {
                    magma_sgeqrf_gpu( M, N, d_A, 0, ldda, tau, dT, 0, opts.queue, &info );
                }
                #ifdef HAVE_CUBLAS
                else if ( opts.version == 3 ) {
                    magma_sgeqrf3_gpu( M, N, d_A, 0, ldda, tau, dT, opts.queue, &info );
                }
                #endif
                else {
            ...
            if ( opts.check == 1 && M >= N ) {
                ...
                magma_sgetmatrix( M, N, d_A, 0, ldda, h_R, M, opts.queue );
                ...
                magma_ssetvector( M, b, 1, d_B, 0, 1, opts.queue );
                ...
                magma_sgetvector( N, d_B, 0, 1, x, 1, opts.queue );
