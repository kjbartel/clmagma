...
int main( int argc, char** argv)
{
    ...
    if (flag != 0) {
        if( num_gpus <= 0 || num_gpus > MagmaMaxGPUs || num_subs <= 0 || num_subs > MagmaMaxSubs ) {
            ...
            printf("\n  testing_sgetrf_mgpu -M %d -N %d -NGPU %d -NSUB %d -check %d\n\n", (int) M, (int) N, (int) num_gpus, (int) num_subs, (int) check);
        }
    } else {
        M = N = size[n_size-1];
        printf("\nDefault: \n");
        printf("  testing_sgetrf_mgpu -M %d -N %d -NGPU %d -NSUB %d -check %d\n\n", (int) M, (int) N, (int) num_gpus, (int) num_subs, (int) check);
    }
    ...
    for (i=0; i<n_size; i++) {
        ...
        TESTING_MALLOC( h_P, float, lda*nb );
       ...
       /* =====================================================================
           ...
           =================================================================== */
        cpu_time = get_time();
        lapackf77_sgetrf(&M, &N, h_R, &lda, ipiv, &info);
        ...
        /* warm-up */
        magma_sgetrf_msub( trans, num_subs, num_gpus, M, N, d_lA, 0, ldda, ipiv, &info, queues);
