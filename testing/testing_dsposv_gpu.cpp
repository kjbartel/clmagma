/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from testing_zcposv_gpu.cpp mixed zc -> ds, Sat Nov 15 00:21:40 2014
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_dsetmatrix( N, N,    h_A, lda, d_A,0, lda, opts.queue );
            magma_dsetmatrix( N, nrhs, h_B, ldb, d_B,0, ldb, opts.queue );
            
            //=====================================================================
            //              Mixed Precision Iterative Refinement - GPU
            //=====================================================================
            gpu_time = magma_wtime();
            magma_dsposv_gpu(opts.uplo, N, nrhs, d_A,0, lda, d_B,0, ldb, d_X,0, ldx,
                             d_workd,0, d_works,0, &posv_iter, opts.queue, &info);
            ...
            //                 Error Computation
            //=====================================================================
            magma_dgetmatrix( N, nrhs, d_X,0, ldx, h_X, ldx, opts.queue ) ;
            ...
            gpu_time = magma_wtime();
            magma_dpotrf_gpu(opts.uplo, N, d_A,0, lda, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
            ...
            gpu_time = magma_wtime();
            magma_dpotrf_gpu(opts.uplo, N, d_A,0, lda, opts.queue, &info);
            magma_dpotrs_gpu(opts.uplo, N, nrhs, d_A,0, lda, d_B,0, ldb, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
            ...
            magma_dsetmatrix( N, N,    h_A, lda, d_A,0, lda, opts.queue );
            magma_dsetmatrix( N, nrhs, h_B, ldb, d_B,0, ldb, opts.queue );
            magmablas_dlag2s( N, N,    d_A,0, lda, d_As,0, N, opts.queue, &info );
            magmablas_dlag2s( N, nrhs, d_B,0, ldb, d_Bs,d_Bs_offset, N, opts.queue, &info );
            
            gpu_time = magma_wtime();
            magma_spotrf_gpu(opts.uplo, N, d_As,0, N, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
            ...
            gpu_time = magma_wtime();
            magma_spotrf_gpu(opts.uplo, N, d_As,0, lda, opts.queue, &info);
            magma_spotrs_gpu(opts.uplo, N, nrhs, d_As,0, N, d_Bs,d_Bs_offset, N, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
