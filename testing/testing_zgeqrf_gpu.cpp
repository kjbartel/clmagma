...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_zsetmatrix( M, N, h_R, lda, d_A, 0, ldda, opts.queue );
            ...
            if ( opts.version == 2 ) {
                magma_zgeqrf2_gpu( M, N, d_A, 0, ldda, tau, opts.queues2, &info );
            }
            else {
                ...
                if ( opts.version == 1 ) {
                    magma_zgeqrf_gpu( M, N, d_A, 0, ldda, tau, dT, 0, opts.queue, &info );
                }
                #ifdef HAVE_CUBLAS
                else if ( opts.version == 3 ) {
                    magma_zgeqrf3_gpu( M, N, d_A, 0, ldda, tau, dT, opts.queue, &info );
                }
                #endif
                else {
            ...
            if ( opts.check == 1 && M >= N ) {
                ...
                magma_zgetmatrix( M, N, d_A, 0, ldda, h_R, M, opts.queue );
                ...
                magma_zsetvector( M, b, 1, d_B, 0, 1, opts.queue );
                ...
                magma_zgetvector( N, d_B, 0, 1, x, 1, opts.queue );
