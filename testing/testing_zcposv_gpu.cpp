...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_zsetmatrix( N, N,    h_A, lda, d_A,0, lda, opts.queue );
            magma_zsetmatrix( N, nrhs, h_B, ldb, d_B,0, ldb, opts.queue );
            
            //=====================================================================
            //              Mixed Precision Iterative Refinement - GPU
            //=====================================================================
            gpu_time = magma_wtime();
            magma_zcposv_gpu(opts.uplo, N, nrhs, d_A,0, lda, d_B,0, ldb, d_X,0, ldx,
            ...
            //                 Error Computation
            //=====================================================================
            magma_zgetmatrix( N, nrhs, d_X,0, ldx, h_X, ldx, opts.queue ) ;
            ...
            gpu_time = magma_wtime();
            magma_zpotrf_gpu(opts.uplo, N, d_A,0, lda, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
            ...
            gpu_time = magma_wtime();
            magma_zpotrf_gpu(opts.uplo, N, d_A,0, lda, opts.queue, &info);
            magma_zpotrs_gpu(opts.uplo, N, nrhs, d_A,0, lda, d_B,0, ldb, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
            ...
            magma_zsetmatrix( N, N,    h_A, lda, d_A,0, lda, opts.queue );
            magma_zsetmatrix( N, nrhs, h_B, ldb, d_B,0, ldb, opts.queue );
            magmablas_zlag2c( N, N,    d_A,0, lda, d_As,0, N, opts.queue, &info );
            magmablas_zlag2c( N, nrhs, d_B,0, ldb, d_Bs,d_Bs_offset, N, opts.queue, &info );
            
            gpu_time = magma_wtime();
            magma_cpotrf_gpu(opts.uplo, N, d_As,0, N, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
            ...
            gpu_time = magma_wtime();
            magma_cpotrf_gpu(opts.uplo, N, d_As,0, lda, opts.queue, &info);
            magma_cpotrs_gpu(opts.uplo, N, nrhs, d_As,0, N, d_Bs,d_Bs_offset, N, opts.queue, &info);
            gpu_time = magma_wtime() - gpu_time;
