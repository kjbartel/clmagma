 ...
 *     @date November 2014
 *
 * @generated from testing_zpotrf_mgpu.cpp normal z -> c, Sat Nov 15 00:21:40 2014
 *
 **/
...
int main( int argc, char** argv)
{
    ...
    if (argc != 1){
        ...
        printf("\nUsage: \n");
        printf("  testing_cpotrf_mgpu -N %d -NGPU %d -UPLO -L\n\n", 1024, num_gpus0);
    }
    ...
    for(i=0; i<num_gpus0; i++){
        TESTING_MALLOC_DEV( d_lA[i], magmaFloatComplex, ldda );
    }
    ...
    for(i=0; i<10; i++){
        ...
        for( int i = 0; i < N; ++i ) {
            h_A(i,i) = MAGMA_C_MAKE( MAGMA_C_REAL(h_A(i,i)) + N, 0 );
            for( int j = 0; j < i; ++j ) {
        ...
        if(uplo == MagmaUpper){
            ...
            for(j=0;j<N;j+=nb){
                k = (j/nb)%num_gpus;
                nk = min(nb, N-j);
                magma_csetmatrix( nk, N, &h_A[j], lda,
                                    d_lA[k], (j/(nb*num_gpus)*nb), ldda,
                                    queues[2*k]);
            }
        }

        magma_cpotrf_mgpu( num_gpus, uplo, N, d_lA, 0, ldda, queues, &info );
