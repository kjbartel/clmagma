 ...
 *     April 2012
 *
 * @generated s Wed Oct 24 00:33:05 2012
 *
 **/
// includes, system
#include <stdlib.h>
...
static int diffMatrix( float *A, float *B, int m, int n, int lda){
    int i, j;
    for(i=0; i<m; i++) {
        for(j=0; j<n; j++)
            if ( !(MAGMA_S_EQUAL( A[lda*j+i], B[lda*j+i] )) ){
...
int main( int argc, char** argv) 
{
    ...
    for(i=0; i<7; i++) {
		...
		lapackf77_slarnv( &ione, ISEED, &n2, h_A1 );
        lapackf77_slacpy( MagmaUpperLowerStr, &M, &N, h_A1, &lda, h_A2, &lda );
		for(r=0;r<M;r++){
			for(c=0;c<N;c++){
		...
		magma_ssetmatrix( N, M, h_AT, 0, ldat, d_A1, 0, ldat, queue);
		...
		if(check_lg !=0){
			...
			magma_sprint(M, N, h_A1, lda);
			printf("gpu swap transpose matrix result:\n");
			magma_sprint(M, N, h_A3, lda);
		}
