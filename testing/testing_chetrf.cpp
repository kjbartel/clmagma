/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from testing_zhetrf.cpp normal z -> c, Sat Nov 15 00:21:40 2014
    ...
    magma_int_t *ipiv )
{
    ...
    // solve Ax = b
    if (nopiv) {
        blasf77_ctrsm( "L", "L", "N", "U", &n, &ione, &c_one,
                       A, &lda, x, &n );
        //for (int i=0; i<n; i++) x[i] /= A[i+i*lda];
        for (int i=0; i<n; i++) x[i] = x[i]/A[i+i*lda]; //MAGMA_C_DIV( x[i], A[i+i*lda] );
        blasf77_ctrsm( "L", "L", "C", "U", &n, &ione, &c_one,
                       A, &lda, x, &n );
    }else {
        chetrs_( (upper ? MagmaUpperStr: MagmaLowerStr), &n, &ione, A, &lda, ipiv, x, &n, &info );
    }
    if (info != 0)
    ...
    norm_A = lapackf77_clange( "F", &n, &n, A, &lda, work );
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lwork = max(N*(1+magma_get_chetrf_nb(N)), (int)MAGMA_C_REAL(temp));
            TESTING_MALLOC_PIN( work, magmaFloatComplex, lwork );
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_chetrf((upper ? MagmaUpperStr: MagmaLowerStr), &N, h_A, &lda, 
            ...
            if (nopiv) {
                ...
                //magma_chetrf_cpu( uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else if (gpu) {
                magma_chetrf( uplo, N, h_A, lda, ipiv, opts.queue, &info);
            } else if (row) {
                //magma_chetrf_gpu_row( uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                //magma_chetrf( uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
