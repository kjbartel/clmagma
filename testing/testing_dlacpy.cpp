/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from testing_zlacpy.cpp normal z -> d, Sat Nov 15 00:21:40 2014
       @author Mark Gates
*/
    ...
    for( int iuplo = 0; iuplo < 3; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_dsetmatrix( M, N, h_B, ldb, d_B, 0, lddb, opts.queue );
            
            gpu_time = magma_sync_wtime( 0 );
            //magmablas_dlacpy( uplo[iuplo], M-2, N-2, d_A+1+ldda, 0, ldda, d_B+1+lddb, 0, lddb, opts.queue );  // inset by 1 row & col
            magmablas_dlacpy( uplo[iuplo], M, N, d_A, 0, ldda, d_B, 0, lddb, opts.queue );
            ...
            if ( opts.verbose ) {
                ...
                printf( "dB=" );  magma_dprint_gpu( M, N, d_B, 0, ldda, opts.queue );
            }
            
            /* =====================================================================
               Check the result
               =================================================================== */
            magma_dgetmatrix( M, N, d_B, 0, ldda, h_R, lda, opts.queue );
