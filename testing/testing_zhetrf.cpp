    ...
    magma_int_t *ipiv )
{
    ...
    if (nopiv) {
        ...
        for (int i=0; i<n; i++) x[i] = x[i]/A[i+i*lda]; //MAGMA_Z_DIV( x[i], A[i+i*lda] );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_zhetrf((upper ? MagmaUpperStr: MagmaLowerStr), &N, h_A, &lda, 
            ...
            if (nopiv) {
                ...
                magma_zhetrf( uplo, N, h_A, lda, ipiv, opts.queue, &info);
            } else if (row) {
