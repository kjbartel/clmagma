 ...
 *     @date November 2014
 *
 * @generated from testing_zpotrf_msub.cpp normal z -> c, Sat Nov 15 00:21:40 2014
 *
 **/
...
void init_matrix( magma_int_t N, magmaFloatComplex *h_A, magma_int_t lda )
{
    ...
    for(i=0; i<10; i++){
        ...
        if (uplo == MagmaUpper) {
            ...
            /*for (j=0; j<N; j+=nb) {
                k = (j/nb)%tot_subs;
                nk = min(nb, N-j);
                magma_csetmatrix( nk, j+nk, &h_R[j], lda,
                                    d_lA[k], j/(nb*tot_subs)*nb, ldda,
                                    queues[2*(k%num_gpus)]);
            }*/
        }
        magma_cpotrf_msub( num_subs, num_gpus, uplo, N, d_lA, 0, ldda, queues, &info );
