 ...
 *     @date November 2013
 *
 * @generated c Mon Nov 25 17:56:09 2013
 *
 **/
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex *h_A, *h_B, *h_C, *h_C2, *h_C3;
    ...
    printf("\nUsage: testing_cgemm [-NN|NT|TN|TT|NC|CN|TC|CT|CC] -M m -N n -K k -count c -l\n"
    ...
    TESTING_MALLOC( h_A,  magmaFloatComplex, lda*K );
    TESTING_MALLOC( h_B,  magmaFloatComplex, ldb*Bn );
    TESTING_MALLOC( h_C,  magmaFloatComplex, ldc*N );
    TESTING_MALLOC( h_C2, magmaFloatComplex, ldc*N );
    TESTING_MALLOC( h_C3, magmaFloatComplex, ldc*N );
    ...
    for( i=istart; i<iend; i = (int)(i*1.25) ) {
        for( int cnt = 0; cnt < count; ++cnt ) {
            ...
            if ( lapack ) {
                cpu_time = get_time();
                blasf77_cgemm( lapack_const(transA), lapack_const(transB), &M, &N, &K,
                               &alpha, h_A, &lda,
                                       h_B, &ldb,
                               &beta,  h_C, &ldc );
            ...
            if ( lapack ) {
                ...
                magma_error = lapackf77_clange("M", &M, &N, h_C2, &ldc, work);
                
                blasf77_caxpy(&szeC, &c_neg_one, h_C, &ione, h_C3, &ione);
                clblas_error = lapackf77_clange("M", &M, &N, h_C3, &ldc, work);
                ...
                blasf77_caxpy(&szeC, &c_neg_one, h_C3, &ione, h_C2, &ione);
                magma_error = lapackf77_clange("M", &M, &N, h_C2, &ldc, work);
