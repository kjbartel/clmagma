/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from testing_zgeqrf_msub.cpp normal z -> c, Sat Nov 15 00:21:40 2014
...
int main( int argc, char** argv)
{
    ...
    printf("Usage: \n");
    printf("  testing_cgeqrf_msub -M %d -N %d -NGPU %d -NSUB %d %s\n\n", M, N, num_gpus, num_subs, (check == 1 ? "-check" : " "));
    ...
    for(i=0; i<10; i++){
        ...
        n2     = lda*N;
        ldda   = ((M+31)/32)*32;
        gflops = FLOPS_CGEQRF( (float)M, (float)N ) / 1e9;
        ...
        lhwork = -1;
        lapackf77_cgeqrf(&M, &N, h_R, &M, tau, tmp, &lhwork, &info);
        ...
        for (int j=0; j<tot_subs; j++) {      
            ...
            TESTING_MALLOC_DEV( d_lA[j], magmaFloatComplex, ldda*n_local[j] );
        }
        ...
        // warm-up
        magma_csetmatrix_1D_col_bcyclic(M, N, h_R, lda, d_lA, ldda, tot_subs, nb, trans_queues);
        magma_cgeqrf_msub(num_subs, num_gpus, M, N, d_lA, ldda, tau, queues, &info);
        ...
        if (info < 0)
          printf("Argument %d of magma_cgeqrf_msub had an illegal value.\n", (int) -info);
        
        if (check == 1) {
            /* =====================================================================
               Check the result compared to LAPACK
               =================================================================== */
            magma_cgetmatrix_1D_col_bcyclic(M, N, d_lA, ldda, h_R, lda, tot_subs, nb, trans_queues);
            ...
            lapackf77_cgeqrf(&M, &N, h_A, &lda, tau, h_work, &lhwork, &info);
