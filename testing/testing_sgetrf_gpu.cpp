/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from testing_zgetrf_gpu.cpp normal z -> s, Sat Nov 15 00:21:40 2014
       @author Mark Gates
*/
// includes, system
#include <stdlib.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 1 ) {
                magma_sgetrf_gpu( M, N, d_A, 0, ldda, ipiv, opts.queue, &info );
            }
            else if ( opts.version == 2 ) {
                magma_sgetrf2_gpu( M, N, d_A, 0, ldda, ipiv, opts.queues2, &info );
            }
            else {
            ...
            if ( opts.check == 2 ) {
                magma_sgetmatrix( M, N, d_A, 0, ldda, h_A, lda, opts.queue );
