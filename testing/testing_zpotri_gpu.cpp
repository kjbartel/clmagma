...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_zpotrf_gpu( opts.uplo, N, d_A, 0, ldda, opts.queue, &info );
            
            // check for exact singularity
            //magma_zgetmatrix( N, N, d_A, 0, ldda, h_R, lda, opts.queue );
            //h_R[ 10 + 10*lda ] = MAGMA_Z_MAKE( 0.0, 0.0 );
            //magma_zsetmatrix( N, N, h_R, lda, d_A, 0, ldda, opts.queue );
            
            gpu_time = magma_wtime();
            magma_zpotri_gpu( opts.uplo, N, d_A, 0, ldda, opts.queues2, &info );
