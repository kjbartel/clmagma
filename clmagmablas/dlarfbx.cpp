/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zlarfbx.cpp normal z -> d, Sat Nov 15 00:21:35 2014
...
magma_dlarfbx_gpu(int m, int k, magmaDouble_ptr V, size_t V_offset, int ldv,
                  magmaDouble_ptr T, size_t T_offset, int ldt, 
                  magmaDouble_ptr c, size_t c_offset, 
                  ...
                  magma_queue_t queue)
{
    ...
    ckKernel = g_runtime.get_kernel( "magma_dgemv_kernel1" );   // in dlarfbx.cl
    if (!ckKernel)
    {
    ...
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&dwork );
    ...
    // launch kernel magma_dgemv_kernel1
    ...
    ckKernel = g_runtime.get_kernel( "magma_dtrmv_tkernel" );   // in dlarfx.cl
    if (!ckKernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
    ...
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&dwork   );
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&dwork_offset );
    magmaDouble_ptr dwork2 = dwork;
    size_t dwork2_offset = dwork_offset + k;
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&dwork2 );
    ...
    // launch kernel magma_dtrmv_tkernel
    ciErrNum = clEnqueueNDRangeKernel(
        queue, ckKernel, 1, NULL, GlobalWorkSize, LocalWorkSize, 0, NULL, NULL);
    if (ciErrNum != CL_SUCCESS)
    {
    ...
    ckKernel = g_runtime.get_kernel( "magma_dgemv_kernel2" );   // in dlarfbx.cl
    if (!ckKernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
    ...
    // launch kernel magma_dgemv_kernel2
    /* c = c - V dwork                */
