/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from ztranspose_inplace.cl normal z -> s, Sat Nov 15 00:21:35 2014
...
__kernel void stranspose_inplace_odd( int n, __global float *matrix, unsigned long matrix_offset, int lda )
{
    matrix += matrix_offset;

    __local float sA[ NB ][ NB+1 ];
    __local float sB[ NB ][ NB+1 ];
    ...
    __global float *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        __global float *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__kernel void stranspose_inplace_even( int n, __global float *matrix, unsigned long matrix_offset, int lda )
{
    matrix += matrix_offset;
