/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from zlaset.cpp normal z -> s, Sat Nov 15 00:21:35 2014
    ...
    magma_queue_t queue)
{
    ...
    if (uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "slaset_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
        ...
        kernel = g_runtime.get_kernel( "slaset_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
        ...
        kernel = g_runtime.get_kernel( "slaset_full" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
