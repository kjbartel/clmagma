/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zgemm_reduce.cl normal z -> d, Sat Nov 15 00:21:35 2014
*/
...
void sum_reduce3d( int n, int j, int k, int i, __local double* x );  // prototype to suppress compiler warning
void sum_reduce3d( int n, int j, int k, int i, __local double* x )
{
...
__kernel 
void magmablas_dgemm_reduce_kernel(int k, double alpha, 
                                   __global double *d_A, int d_A_offset, int lda,
                                   __global double *d_B, int d_B_offset, int ldb,
                                   double beta,
                                   __global double *d_C, int d_C_offset, int ldc)
{
        d_A += d_A_offset;
        d_B += d_B_offset;
        d_C += d_C_offset;
        ...
        */
        ...
        __local double sum[BLK_M][BLK_N+1][ BLK_K +1];
        double lsum;

        /*  w := v' * C  */
        lsum = MAGMA_D_ZERO;
        for( int j = i; j < k; j += BLK_K )
            lsum += MAGMA_D_CNJG( d_A[j] )* d_B[j];
        
        sum[get_local_id(1)][get_local_id(2)][i] = lsum;
        sum_reduce3d(BLK_K,  get_local_id(1), get_local_id(2), i, (__local double*) sum );
