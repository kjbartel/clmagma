/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zlarfbx.cpp normal z -> s, Sat Nov 15 00:21:35 2014
                  ...
                  magma_queue_t queue)
{
    ...
    ckKernel = g_runtime.get_kernel( "magma_sgemv_kernel1" );   // in slarfbx.cl
    if (!ckKernel)
    {
    ...
    ckKernel = g_runtime.get_kernel( "magma_strmv_tkernel" );   // in slarfx.cl
    if (!ckKernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
    ...
    ckKernel = g_runtime.get_kernel( "magma_sgemv_kernel2" );   // in slarfbx.cl
    if (!ckKernel)
    {
        printf ("Error: cannot locate kernel in line %d, file %s\n", __LINE__, __FILE__);
        return MAGMA_ERR_UNKNOWN;
