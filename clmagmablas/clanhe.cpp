/*
    -- MAGMA (version 1.3.0) --
       ...
       @author Mark Gates
       @generated from zlanhe.cpp normal z -> c, Sat Nov 15 00:21:35 2014

       auto-converted from clanhe.cu
...
clanhe_inf(
    ...
    magmaFloatComplex_const_ptr A, size_t A_offset, int lda,
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "clanhe_inf_kernel_generic_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
        ...
        kernel = g_runtime.get_kernel( "clanhe_inf_kernel_generic_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(n           ), &n            );
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        kernel = g_runtime.get_kernel( "clanhe_max_kernel_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(n           ), &n            );
        ...
        kernel = g_runtime.get_kernel( "clanhe_max_kernel_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(n           ), &n            );
    ...
    magma_queue_t queue )
{
    ...
    float res = 0;
    if ( inf_norm ) {
        clanhe_inf( uplo, n, dA,dA_offset, ldda, dwork,dwork_offset, queue );
    }
    else {
        clanhe_max( uplo, n, dA,dA_offset, ldda, dwork,dwork_offset, queue );
    }
    int i = magma_isamax( n, dwork,dwork_offset, 1, queue ) - 1;
