/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zgemm_reduce.cpp normal z -> d, Sat Nov 15 00:21:35 2014
...
/*  -- clMAGMA (version 1.3.0) --
   ...
   =====================================================================    */
    ...
    if (m%BLK_M!=0 || n%BLK_N!=0) {
        printf("dgemm_reduce works only for m and n divisible by \n");
        printf("correspondingly %d and %d. Calling magma_dgemm ...\n.", 
               BLK_M, BLK_N);
        ...
        ckKernel = g_runtime.get_kernel( "magmablas_dgemm_reduce_kernel" );
        if (!ckKernel)
        {
        ...
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(double), (void*)&alpha );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&d_A     );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&d_A_offset   );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&lda );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&d_B     );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&d_B_offset   );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&ldb );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(double), (void*)&beta     );
        ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&d_C     );
