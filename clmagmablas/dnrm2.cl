/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from dznrm2.cl normal z -> d, Sat Nov 15 00:21:35 2014
*/
...
void sum_reduce(int n,  int i, __local double* x );  // prototype to suppress compiler warning
...
//==============================================================================
__kernel void
magmablas_dnrm2_kernel( int m, __global double *da, int da_offset, int ldda, __global double *dxnorm, int dxnorm_offset )
{
    da += da_offset;
    dxnorm += dxnorm_offset;
    const int i = get_local_id(0);
    
    //double *dx = da + get_group_id(0) * ldda;
...
//        double im = MAGMA_D_IMAG( dx[j] );
        re = MAGMA_D_REAL( da[j] );
        double im = MAGMA_D_IMAG( da[j] );
        lsum += re*re + im*im;
#endif
...
__kernel void
magmablas_dnrm2_adjust_kernel(__global double *xnorm, int xnorm_offset, __global double *c, int c_offset)
{
   xnorm += xnorm_offset;
   c += c_offset;
