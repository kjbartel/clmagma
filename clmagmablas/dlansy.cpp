/*
    -- MAGMA (version 1.3.0) --
       ...
       @author Mark Gates
       @generated from zlanhe.cpp normal z -> d, Sat Nov 15 00:21:35 2014

       auto-converted from dlansy.cu
...
dlansy_inf(
    ...
    magmaDouble_const_ptr A, size_t A_offset, int lda,
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "dlansy_inf_kernel_generic_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
        ...
        kernel = g_runtime.get_kernel( "dlansy_inf_kernel_generic_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(n           ), &n            );
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        kernel = g_runtime.get_kernel( "dlansy_max_kernel_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(n           ), &n            );
        ...
        kernel = g_runtime.get_kernel( "dlansy_max_kernel_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(n           ), &n            );
