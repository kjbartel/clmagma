/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from zlacpy.cpp normal z -> d, Sat Nov 15 00:21:35 2014
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        kernel = g_runtime.get_kernel( "dlacpy_kernel_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
        ...
        kernel = g_runtime.get_kernel( "dlacpy_kernel_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
        ...
        kernel = g_runtime.get_kernel( "dlacpy_kernel_full" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
...
magmablas_dlacpy_batched(
    magma_uplo_t uplo, magma_int_t m, magma_int_t n,
    magmaDouble_const_ptr const *dAarray, size_t dA_offset, magma_int_t ldda,
    magmaDouble_ptr             *dBarray, size_t dB_offset, magma_int_t lddb,
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        kernel = g_runtime.get_kernel( "dlacpy_kernel_batched_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
        ...
        kernel = g_runtime.get_kernel( "dlacpy_kernel_batched_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
        ...
        kernel = g_runtime.get_kernel( "dlacpy_kernel_batched_full" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
