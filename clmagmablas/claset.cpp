/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from zlaset.cpp normal z -> c, Sat Nov 15 00:21:35 2014
    ...
    magma_queue_t queue)
{
    ...
    if (uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "claset_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
        ...
        kernel = g_runtime.get_kernel( "claset_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
        ...
        kernel = g_runtime.get_kernel( "claset_full" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(m        ), &m         );
