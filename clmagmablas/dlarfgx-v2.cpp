/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zlarfgx-v2.cpp normal z -> d, Sat Nov 15 00:21:35 2014
...
magma_dlarfgx_gpu(int n, magmaDouble_ptr dx0, size_t dx0_offset, magmaDouble_ptr dx, size_t dx_offset,  
                  ...
                  magma_queue_t queue)
{
    ...
    ckKernel = g_runtime.get_kernel( "magma_dlarfgx_gpu_kernel" );
    if (!ckKernel)
    {
    ...
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&dx0 );
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&dx0_offset     );
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&dx   );
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&dx_offset );
    ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&dtau     );
...
magma_dlarfgtx_gpu(int n, magmaDouble_ptr dx0, size_t dx0_offset, magmaDouble_ptr dx, size_t dx_offset,
                   magmaDouble_ptr dtau, size_t dtau_offset, magmaDouble_ptr dxnorm, size_t dxnorm_offset, 
                   magmaDouble_ptr dA, size_t dA_offset, int i, 
                   magmaDouble_ptr V, size_t V_offset, int ldv, magmaDouble_ptr T, size_t T_offset, int ldt, 
                   magmaDouble_ptr work, size_t work_offset, 
                   magma_queue_t queue)
{
   /*  Generate the elementary reflector H(i)  */
   magma_dlarfgx_gpu(n, dx0, dx0_offset, dx, dx_offset, dtau, dtau_offset, dxnorm, dxnorm_offset, dA, dA_offset, i, queue);

   if (i==0){
      double tt = MAGMA_D_ONE;
      magmablas_dlacpy(MagmaFull, 1, 1, dtau, dtau_offset, 1, T, T_offset+i+i*ldt, 1, queue);
      magma_dsetmatrix(1, 1, &tt, 1, dx0, dx0_offset, 1, queue);
   }
   else
   {
      ...
      cl_int ciErrNum;                // Error code var
      cl_kernel ckKernel=NULL;
      ckKernel = g_runtime.get_kernel( "magma_dgemv_kernel3" );     // in dlarfbx.cl
      ...
      int nn = 0;
      ciErrNum  = clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&n   );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&V );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&V_offset     );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&ldv   );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&dx0 );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&dx0_offset     );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&work );
      ...
      ckKernel = g_runtime.get_kernel( "magma_dtrmv_kernel2" );         // in dlarfx.cl
      ...
      nn = 0;
      ciErrNum  = clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&T   );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&T_offset     );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&ldt   );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&work );
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(int), (void*)&work_offset     );
      magmaDouble_ptr T1 = T;
      size_t T1_offset = T_offset + i*ldt;
      ciErrNum |= clSetKernelArg( ckKernel, nn++, sizeof(magmaDouble_ptr), (void*)&T1 );
