/*
    -- MAGMA (version 1.3.0) --
       ...
       @author Mark Gates
       @generated from zlanhe.cpp normal z -> s, Sat Nov 15 00:21:35 2014
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "slansy_inf_kernel_generic_lower" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
        ...
        kernel = g_runtime.get_kernel( "slansy_inf_kernel_generic_upper" );
        if ( kernel != NULL ) {
            err = 0;
            i   = 0;
            err |= clSetKernelArg( kernel, i++, sizeof(n           ), &n            );
