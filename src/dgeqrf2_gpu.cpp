/*
    -- clMAGMA (version 1.3.0) --
       ...
       @author Stan Tomov
       @generated from zgeqrf2_gpu.cpp normal z -> d, Sat Nov 15 00:21:37 2014
    ...
    */
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k-nx; i += nb) {
            ...
            magma_dgetmatrix_async(rows, ib, dA(i, i), ldda, work(i), ldwork, queue[0], NULL);
            ...
            if (i > 0) {
                ...
                magma_dsetmatrix_async( old_ib, old_ib, work(old_i), ldwork,
            ...
            dpanel_to_q( MagmaUpper, ib, work(i), ldwork, hwork+ib*ib );

            /* download the i-th V matrix */
            magma_dsetmatrix_async(rows, ib, work(i), ldwork, dA(i,i), ldda, queue[0], NULL);

            /* download the T matrix */
            magma_queue_sync( queue[1] );
            magma_dsetmatrix_async( ib, ib, hwork, ib, dwork, 0, lddwork, queue[0], NULL);
            magma_queue_sync( queue[0] );
            ...
            if (i + ib < n) {
                if (i+nb < k-nx) {
                    ...
                    dq_to_panel( MagmaUpper, ib, work(i), ldwork, hwork+ib*ib );
                }
                else {
                    magma_dlarfb_gpu( MagmaLeft, MagmaConjTrans, MagmaForward, MagmaColumnwise,
                                      rows, n-i-ib, ib,
                                      ...
                                      dA(i, i+ib), ldda, dwork,ib, lddwork, queue[1]);
                    dq_to_panel( MagmaUpper, ib, work(i), ldwork, hwork+ib*ib );
                    magma_dsetmatrix_async(ib, ib, work(i), ldwork, dA(i,i), ldda, queue[1], NULL);
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_dgetmatrix_async(rows, ib, dA(i, i), ldda, work, rows, queue[1], NULL);
        magma_queue_sync(queue[1]);
        
        lhwork = lwork - rows*ib;
        lapackf77_dgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_dsetmatrix_async(rows, ib, work, rows, dA(i, i), ldda, queue[1], NULL);
    }
