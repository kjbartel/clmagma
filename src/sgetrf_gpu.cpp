/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zgetrf_gpu.cpp normal z -> s, Sat Nov 15 00:21:37 2014
...
/*  -- clMAGMA (version 1.3.0) --
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        magma_sgetmatrix( m, n, dA(0,0), ldda, work(0), m, queue );
        lapackf77_sgetrf( &m, &n, work, &m, ipiv, info );
        magma_ssetmatrix( m, n, work(0), m, dA(0,0), ldda, queue );
        magma_free_cpu( work );
    }
    else {
        ...
        if ( m == n ) {
            ...
            lddat = ldda;
            magmablas_stranspose_inplace( m, dAT(0,0), lddat, queue );
        }
        else {
            lddat = maxn;  // N-by-M
            dAT_offset = 0;
        ...
        for( j=0; j < s; j++ ) {
            // download j-th panel
            magmablas_stranspose( nb, m-j*nb, dAT(j,j), lddat, dAP(0,0), maxm, queue );
            magma_sgetmatrix( m-j*nb, nb, dAP(0,0), maxm, work(0), ldwork, queue );
            ...
            magmablas_slaswp( n, dAT(0,0), lddat, j*nb + 1, j*nb + nb, ipiv, 1, queue );

            // upload j-th panel
            magma_ssetmatrix( m-j*nb, nb, work(0), ldwork, dAP(0,0), maxm, queue );
            magmablas_stranspose( m-j*nb, nb, dAP(0,0), maxm, dAT(j,j), lddat, queue );
        ...
        if ( nb0 > 0 ) {
            ...
            magmablas_stranspose( nb0, rows, dAT(s,s), lddat, dAP(0,0), maxm, queue );
            magma_sgetmatrix( rows, nb0, dAP(0,0), maxm, work(0), ldwork, queue );
            ...
            magmablas_slaswp( n, dAT(0,0), lddat, s*nb + 1, s*nb + nb0, ipiv, 1, queue );
    
            // upload j-th panel
            magma_ssetmatrix( rows, nb0, work(0), ldwork, dAP(0,0), maxm, queue );
            magmablas_stranspose( rows, nb0, dAP(0,0), maxm, dAT(s,s), lddat, queue );
