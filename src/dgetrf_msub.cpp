...
/*  -- clMAGMA (version 1.1.0-beta2) --
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        magma_dgetmatrix( m, n, d_lA[0], 0, ldda, work, 0, m, queues[0] );
        lapackf77_dgetrf(&m, &n, work, &m, ipiv, info);
        magma_dsetmatrix( m, n, work, 0, m, d_lA[0], 0, ldda, queues[0] );
        magma_free_cpu(work);
    } else {
        ...
        for (i=0; i<tot_subs; i++) {
            ...
            if (trans == MagmaNoTrans) {
                ...
                if ((m == n_local[i]) && (m%32 == 0) && (ldda%32 == 0) && (ldda == lddat)) {
                    d_lAT[i] = d_lA[i];
                    magma_dinplace_transpose( d_lA[i], 0, ldda, ldda, queues[2*(i%num_gpus)+1] );
                } else {
                    #ifdef SINGLE_GPU_PER_CONTEXT
                    ...
                    magma_dtranspose2(d_lAT[i], 0, lddat, d_lA[i], 0, ldda, m, n_local[i], queues[2*(i%num_gpus)+1]);
                }
            }
        }
        if (trans == MagmaNoTrans) {
        ...
        for (d=0; d<tot_subs; d++) {
            if (trans == MagmaNoTrans) {
                ...
                if (d_lAT[d] == d_lA[d]) {
                    magma_dinplace_transpose( d_lA[d], 0, ldda, ldda, queues[2*(d%num_gpus)+1] );
                } else {
                    magma_dtranspose2( d_lA[d], 0, ldda, d_lAT[d], 0, lddat, n_local[d], m, queues[2*(d%num_gpus)+1] );
                }
            }
        }
        /* clean up */
