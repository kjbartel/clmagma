...
/*  -- clMAGMA (version 1.0.0) --
    ...
    *info = 0;
    ...
    anrm = lapackf77_zlanhe("M", lapack_const(uplo_), &n, a, &lda, rwork);
    iscale = 0;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
        sigma = rmin / anrm;
    ...
    if (iscale == 1) {
        lapackf77_zlascl(lapack_const(uplo_), &izero, &izero, &d_one, &sigma, &n, &n, a,
                         &lda, info);
    }
...
//#define ENABLE_TIMER
    ...
    magma_zhetrd(lapack_const(uplo)[0], n, a, lda, w, &rwork[inde],
    ...
    if (! wantz) {
        ...
        magma_zstedx(MagmaAllVec, n, 0., 0., 0, 0, w, &rwork[inde],
