/*
     -- clMAGMA (version 1.3.0) --
        ...
        @generated from zpotrf_gpu.cpp normal z -> s, Sat Nov 15 00:21:37 2014
...
#define A(i,j)  &A[ (i) + (j)*lda ]
    ...
    *info = 0;
    ...
    if ((nb <= 1) || (nb >= n)) {
        // use unblocked code
        magma_sgetmatrix( n, n, dA, dA_offset, ldda, work, n, queue );
        lapackf77_spotrf( lapack_uplo_const(uplo), &n, work, &n, info );
        magma_ssetmatrix( n, n, work, n, dA, dA_offset, ldda, queue );
    }
    else {
        if ( uplo == MagmaUpper ) {
            // --------------------
            ...
            for( j = 0; j < n; j += nb ) {
                ...
                magma_sgetmatrix_async( jb, jb, dA(j,j), ldda, work, jb, queue, &event );
                ...
                magma_ssetmatrix_async( jb, jb, work, jb, dA(j,j), ldda, queue, &event );
