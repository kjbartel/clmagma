/*
   ...
   @generated from zgetri_gpu.cpp normal z -> c, Sat Nov 15 00:21:37 2014
...
magma_cgetri_gpu(
    magma_int_t n, magmaFloatComplex_ptr dA, size_t dA_offset, magma_int_t ldda,
...
/*  -- clMAGMA (version 1.3.0) --
    ...
    *info = 0;
    ...
    magma_ctrtri_gpu( MagmaUpper, MagmaNonUnit, n, dA, 0, ldda, queues, info );
    if ( *info != 0 )
        return *info;
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magmablas_claset( MagmaLower, n-j, jb, c_zero, c_zero, dA(j,j), ldda, queues[0] );
