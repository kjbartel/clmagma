...
/*  -- clMAGMA (version 1.0.0) --
    ...
    *info = 0;
    ...
    size_t da_offset = 0;
    if (MAGMA_SUCCESS != magma_malloc( &da, (N*ldda + 2*N*nb + nb*nb)*sizeof(magmaDoubleComplex) )) {
    ...
    t = (magmaDoubleComplex*) malloc(nb*nb*sizeof(magmaDoubleComplex));
    if ( t == NULL ) {
        magma_free( da );
        *info = MAGMA_ERR_HOST_ALLOC;
        return *info;
    }
	magmaDoubleComplex_ptr d_t;
    d_t = d_work;
	size_t d_t_offset = d_work_offset+nb*ldda;

    zzero_nbxnb_block(nb, d_A, d_A_offset+N*ldda, ldda, queue);
    ...
    if (nb < nbmin || nb >= nh) {
      ...
      magma_zsetmatrix( N, N-ilo+1, a+(ilo-1)*(lda), 0, lda, d_A, d_A_offset, ldda, queue );
      ...
      for (i__ = ilo; i__ < ihi - nb; i__ += nb) {
        ...
        magma_zsetmatrix( nb, nb, t, 0, nb, d_t, d_t_offset, nb, queue );
