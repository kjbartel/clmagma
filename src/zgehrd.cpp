...
/*  -- clMAGMA (version 1.3.0) --
    ...
    *info = 0;
    ...
    magmablas_zlaset( MagmaFull, nb, nb, c_zero, c_zero, d_A, d_A_offset+N*ldda, ldda, queue );
    ...
    if (nb < nbmin || nb >= nh) {
      ...
      magma_zsetmatrix( N, N-ilo+1, a+(ilo-1)*(lda), lda, d_A, d_A_offset, ldda, queue );
      ...
      for (i__ = ilo; i__ < ihi - nb; i__ += nb) {
        ...
        magma_zsetmatrix( nb, nb, t, nb, d_t, d_t_offset, nb, queue );
