/*
    -- clMAGMA (version 1.0.0) --
       ...
       @generated c Wed Oct 24 00:32:47 2012
...
/*  -- clMAGMA (version 1.0.0) --
    ...
    *info = 0;
    ...
    if ( MAGMA_SUCCESS != magma_malloc( &dwork, n*nb*sizeof(magmaFloatComplex))) {
        *info = MAGMA_ERR_DEVICE_ALLOC;
        return *info;
    }
    
    if ( MAGMA_SUCCESS != magma_malloc_host((void**)&work, lwork*sizeof(magmaFloatComplex)) ) {
    ...
    if (nb >= nbmin && nb < k && nx < k) {
	    ...
	    magma_cgetmatrix_async(rows, ib, dA(i, i), ldda, work_ref(i), 0, ldwork, queue, &event[0]);
	    ...
	    magma_csetmatrix(rows, ib, work_ref(i), 0, ldwork, dA(i,i), ldda, queue);
            cq_to_panel( MagmaUpper, ib, work_ref(i), ldwork, hwork+ib*ib );
	    
            if (i + ib < n) 
	      {
		magma_csetmatrix(ib, ib, hwork, 0, ib, dwork, 0, lddwork, queue);
                ...
                if (i+nb < k-nx)
                    ...
                    magma_clarfb_gpu( MagmaLeft, MagmaConjTrans, MagmaForward, MagmaColumnwise,
                                      ...
                                      dA(i, i   ), ldda, dwork,0,  lddwork, 
                                      dA(i, i+ib), ldda, dwork,ib, lddwork, queue);
		    magma_csetmatrix(ib, ib, work_ref(i), 0, ldwork, dA(i,i), ldda, queue);
                }
                old_i  = i;
                old_ib = ib;
	      }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
	magma_cgetmatrix(rows, ib, dA(i, i), ldda, work, 0, rows, queue);

        lhwork = lwork - rows*ib;
        lapackf77_cgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
	magma_csetmatrix(rows, ib, work, 0, rows, dA(i, i), ldda, queue);
    }
