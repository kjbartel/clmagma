/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zpotrf2_msub.cpp normal z -> s, Sat Nov 15 00:21:37 2014
    ...
    *info = 0;
    ...
    if (uplo == MagmaUpper) {
        ...
        for (j=0; j<n; j+=nb) {
            ...
            trace_cpu_end(0);
            if (*info != 0) {
                printf( " spotrf returned %d (id=%d,j=%d,j_local=%d,jb=%d)\n",*info,id,j,j_local,jb );
                *info = *info + j;
                break;
            }
    ...
    trace_finalize("spotrf_msub.svg", "trace.css");
    return *info;
} /* magma_spotrf2_msub */
