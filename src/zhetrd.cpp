...
/*  -- clMAGMA (version 1.0.0) --
    ...
    *info = 0;
	...
	size_t da_offset = 0;
    if (MAGMA_SUCCESS != magma_malloc( &da, (n*ldda + 2*n*nb )*sizeof(magmaDoubleComplex))) {
    ...
    if (upper) {
        ...
        magma_zsetmatrix( n, n, A(0, 0), 0, lda, dA(0, 0), ldda, queue );
        ...
        for (i = n - nb; i >= kk; i -= nb) 
          {
            ...
            if (i!=n-nb)
              magma_zgetmatrix( i+nb, nb, dA(0, i), ldda, A(0, i), 0, lda, queue );
            ...
            magma_zsetmatrix( i + nb, nb, work, 0, ldwork, dwork, dwork_offset, lddwork, queue );

            magma_zher2k(magma_uplo_const(uplo), MagmaNoTrans, i, nb, c_neg_one, 
        ...
        magma_zgetmatrix( kk, kk, dA(0, 0), ldda, A(0, 0), 0, lda, queue );
    ...
    else 
      {
        ...
        if (MAGMA_SUCCESS != magma_malloc( &dwork2, (n*n)*sizeof(magmaDoubleComplex) )) {
            *info = MAGMA_ERR_DEVICE_ALLOC;
            return *info;
        }
        ...
        for (i = 0; i < n-nx; i += nb) 
          {
            ...
            /*   Get the current panel (no need for the 1st iteration) */
            if (i!=0)
              magma_zgetmatrix( n-i, nb, dA(i, i), ldda, A(i, i), 0, lda, queue );
            #ifdef FAST_HEMV
			// unported
            ...
            magma_zsetmatrix( n-i, nb, work, 0, ldwork, dwork, dwork_offset, lddwork, queue );
        ...
        /* Use unblocked code to reduce the last or only block */
        if (1<=n-nx)
          magma_zgetmatrix( n-i, n-i, dA(i, i), ldda, A(i, i), 0, lda, queue );
        i_n = n-i;
