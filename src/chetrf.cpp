/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from zhetrf.cpp normal z -> c, Sat Nov 15 00:21:37 2014
*/
    ...
    magma_queue_t queue, magma_int_t *info)
{
    ...
    magma_int_t            iinfo = 0, nk, kb, dA_offset = 0;
    ...
    *info = 0;
    ...
    if ( upper ) {
         for (int k = 0; k < n; k+=nb ) {
              kb = min(nb, n-k);
              magma_csetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, queue, &event);
         }
    } else {
         for (int k = 0; k < n; k+=nb ) {
              kb = min(nb, n-k);
              magma_csetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, queue, &event);
         }
    }
         ...
         for (int k = n-1; k >= 0; k-=kb ) {
              ...
              if ( k+1 > nb )
              {
                   ...
                   magma_clahef_gpu( MagmaUpper, nk, kb, &kb, A(0,0), lda, dA(0,0), ldda,
                                     &ipiv[0], dW,0, ldda, queue, &iinfo );
               } else {

                   /* Use unblocked code to factorize columns 1:k of A */
                   magma_cgetmatrix( nk, nk, dA( 0, 0 ),ldda, A(0,0), lda, queue );
         ...
         for (int k = 0; k < n; k+=kb ) {
             ...
             if ( k < n-nb ) {
                 ...
                 magma_clahef_gpu( MagmaLower, nk, nb, &kb, A( k, k ), lda, dA( k, k ), ldda, 
                                   &ipiv[k], dW,0, ldda, queue, &iinfo );

             } else {
                 /* Use unblocked code to factorize columns k:n of A */
                 magma_cgetmatrix( nk, nk, dA(k,k), ldda, A(k,k), lda, queue );
