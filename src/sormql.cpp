/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zunmql.cpp normal z -> s, Sat Nov 15 00:21:37 2014
...
/*  -- MAGMA (version 1.3.0) --
    ...
    magma_ssetmatrix( m, n, c, ldc, dc, 0, m, queue );
    //dc -= (1 + m);
    ...
    *info  = 0;
    ...
    if ( nb >= k )
      {
        /* Use CPU code */
        lapackf77_sormql(lapack_const(side), lapack_const(trans), &m, &n, &k, &a[a_offset], &lda, &tau[1],
    ...
    else
      {
        ...
        for (i__ = i1; i3 < 0 ? i__ >= i2 : i__ <= i2; i__ += i3) {
          ...
          /* 1) Put 0s in the lower triangular part of A;
             ...
             3) restore A                                      */
          spanel_to_q(MagmaLower, ib, &a[i__ + i__ * lda], lda, t+ib*ib);
          magma_ssetmatrix( i__4, ib, &a[1 + i__ * lda], lda, dwork, 0, i__4, queue );
          sq_to_panel(MagmaLower, ib, &a[i__ + i__ * lda], lda, t+ib*ib);
          ...
          magma_ssetmatrix( ib, ib, t, ib, dwork, i__4*ib, ib, queue );
        ...
        magma_sgetmatrix( m, n, dc, dc_offset+(1+m), m, &c[c_offset], ldc, queue );
    }
    work[0] = MAGMA_S_MAKE( lwkopt, 0 );
