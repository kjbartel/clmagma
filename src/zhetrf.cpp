    ...
    *info = 0;
    ...
    if ( upper ) {
         for (int k = 0; k < n; k+=nb ) {
              kb = min(nb, n-k);
              magma_zsetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, queue, &event);
         }
    } else {
         for (int k = 0; k < n; k+=nb ) {
              kb = min(nb, n-k);
              magma_zsetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, queue, &event);
         }
    }
    ...
    if( upper ) {
         ...
         for (int k = n-1; k >= 0; k-=kb ) {
              ...
              if ( k+1 > nb )
              {
                   ...
                   magma_zgetmatrix( nk, nk, dA( 0, 0 ),ldda, A(0,0), lda, queue );
        ...
        /* Factorize A as L*D*L' using the lower triangle of A
         ...
         for (int k = 0; k < n; k+=kb ) {
             ...
             if ( k < n-nb ) {
                 ...
                 magma_zgetmatrix( nk, nk, dA(k,k), ldda, A(k,k), lda, queue );
