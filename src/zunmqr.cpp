...
/*  -- MAGMA (version 1.0.0) --
    ...
    magma_malloc( &dc, (m)*(n)*sizeof(magmaDoubleComplex) );
    magma_malloc( &dwork, (m + n + 64)*64*sizeof(magmaDoubleComplex) );
    ...
    *info = 0;
    ...
    if (nb >= k) 
      {
        /* Use CPU code */
        lapackf77_zunmqr(lapack_const(side_), lapack_const(trans_), &m, &n, &k, &a[a_offset], &lda, &tau[1],
        ...
        for (i__ = i1; i3 < 0 ? i__ >= i2 : i__ <= i2; i__ += i3) 
          {
            ...
            magma_zsetmatrix( i__4, ib, &a[i__ + i__ * lda], 0, lda, dwork, 0, i__4, queue );
