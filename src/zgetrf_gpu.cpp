...
#define inAT(i,j) dAT, dAT_offset + (i)*nb*lddat + (j)*nb
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) 
      {
        // use CPU code
        err = magma_malloc_host( (void**) &work, m*n*sizeof(magmaDoubleComplex) );
    ...
    else 
      {
        ...
        if ( MAGMA_SUCCESS != magma_malloc( &dAP, nb*maxm*sizeof(magmaDoubleComplex))) {
        ...
        if ((m == n) && (m % 32 == 0) && (ldda%32 == 0))
          {
            ...
            magma_zinplace_transpose( dAT, dAT_offset, ldda, lddat, queue );
          }
        else 
          {
            dAT_offset = 0;
            if ( MAGMA_SUCCESS != magma_malloc( &dAT, maxm*maxn*sizeof(magmaDoubleComplex))) {
              magma_free( dAP );
              *info = MAGMA_ERR_DEVICE_ALLOC;
              return *info;
            }
        ...
        if ( MAGMA_SUCCESS != magma_malloc_host((void**)&work, maxm*nb*sizeof(magmaDoubleComplex)) ) {
          magma_free( dAP );
          if (! ((m == n) && (m % 32 == 0) && (ldda%32 == 0)) )
            magma_free( dAT );
        ...
        if ((m == n) && (m % 32 == 0) && (ldda%32 == 0)) {
          magma_zinplace_transpose( dAT, dAT_offset, lddat, ldda, queue );
        }
        else {
