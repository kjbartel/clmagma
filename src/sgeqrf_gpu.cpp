/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zgeqrf_gpu.cpp normal z -> s, Sat Nov 15 00:21:37 2014
*/
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            magma_ssetmatrix( rows, ib, work_ref(i), ldwork, a_ref(i,i), ldda, queue);
            
            if (i + ib < n) {
                /* Send the triangular factor T to the GPU */
                magma_ssetmatrix( ib, ib, hwork, ib, t_ref(i), nb, queue );
                ...
                if (i+nb < k-nb){
                    ...
                    magma_ssetmatrix( ib, ib, ut, ib, d_ref(i), ib , queue);
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_sgetmatrix( rows, ib, a_ref(i, i), ldda, work, rows, queue );
        lhwork = lwork - rows*ib;
        lapackf77_sgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_ssetmatrix( rows, ib, work, rows, a_ref(i, i), ldda, queue );
    }
