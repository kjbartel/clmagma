    ...
    if (nb <= 1 || nb >= n) {
        ...
        for (i=0; i < tot_subs; i++) {
            ...
            if (trans == MagmaNoTrans) {
                ...
                if ( m == n_local[i] ) {
                    d_lAT[i] = d_lA[i];
                    magmablas_ztranspose_inplace( m, d_lA[i], 0, ldda, queues[2*(i%num_gpus)+1] );
                } else {
                    #ifdef SINGLE_GPU_PER_CONTEXT
                    ...
                    magmablas_ztranspose( m, n_local[i], d_lA[i], 0, ldda, d_lAT[i], 0, lddat, queues[2*(i%num_gpus)+1]);
                }
            }
        }
        if (trans == MagmaNoTrans) {
        ...
        for (d=0; d < tot_subs; d++) {
            if (trans == MagmaNoTrans) {
                ...
                if (d_lAT[d] == d_lA[d]) {
                    magmablas_ztranspose_inplace( m, d_lA[d], 0, ldda, queues[2*(d%num_gpus)+1] );
                } else {
                    magmablas_ztranspose( n_local[d], m, d_lAT[d], 0, lddat, d_lA[d], 0, ldda, queues[2*(d%num_gpus)+1] );
                }
            }
        }
        /* clean up */
