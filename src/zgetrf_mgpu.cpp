    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        for(i=0; i<num_gpus; i++){
            ...
            magma_ztranspose2(d_lAT[i], 0, lddat, d_lA[i], 0, ldda, m, n_local[i], queues[2*i+1]);
          }
          for(i=0; i<num_gpus; i++){
          ...
          /* cpu workspace */
          lddwork = maxm;
          if (MAGMA_SUCCESS != magma_zmalloc_cpu( &work, lddwork*nb*num_gpus )) {
              for(i=0; i<num_gpus; i++ ) {
          ...
          magma_zgetrf2_mgpu(num_gpus, m, n, nb, 0, d_lAT, 0, lddat, ipiv, d_panel, 0, work, maxm,
                             info, queues);

          /* clean up */
          for( d=0; d<num_gpus; d++ ) {
              /* save on output */
              magma_ztranspose2( d_lA[d], 0, ldda, d_lAT[d], 0, lddat, n_local[d], m, queues[2*d+1] );
