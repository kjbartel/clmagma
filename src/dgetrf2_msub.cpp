...
magma_err_t
magma_dtranspose_host( int flag, int d,
    magmaDouble_ptr odata, int offseto, int ldo,
    magmaDouble_ptr idata, int offseti, int ldi,
    int m, int n,
    magma_queue_t queue )
{
    int i, j;
    double *work1, *work2;
    if (MAGMA_SUCCESS != magma_dmalloc_cpu( &work1, m*n )) {
        return MAGMA_ERR_HOST_ALLOC;
    }
    if (MAGMA_SUCCESS != magma_dmalloc_cpu( &work2, m*n )) {
        return MAGMA_ERR_HOST_ALLOC;
    }
    /* download to CPU */
...
#define W(j) (w+((j)%(1+num_gpus))*nb*ldw)
    ...
    magmaDouble_ptr panel_local[MagmaMaxSubs * MagmaMaxGPUs];
    ...
    *info = -5;
    ...
    if (tot_subs > ceil((double)n/nb)) {
      *info = -1;
      ...
      nb0 = min(mindim, nb);
      if (nb0 == nb) {
        magma_dtranspose(  d_lAP[0], dlAP_offset, maxm, inAT(0,0,0), lddat, nb0, maxm, queues[2*0+1] );
      } else {
        magma_dtranspose2( d_lAP[0], dlAP_offset, maxm, inAT(0,0,0), lddat, nb0, maxm, queues[2*0+1] );
      }
      ...
      for (i=0; i<s; i++) {
          ...
          for (dd=0; dd<tot_subs; dd++) {
              if (dd == 0) {
                ...
                magma_dpermute_long2( lddat, inAT(d,0,0), lddat, ipiv, nb, i*nb, queues[2*(d%num_gpus)] );
              } else {
                // ipiv is already added by row offset, calling long3   
                magma_dpermute_long3( lddat, inAT(d,0,0), lddat, ipiv, nb, i*nb, queues[2*(d%num_gpus)] );
              }
              d = (d+1)%tot_subs;
          }
          ...
          for (dd=0; dd<tot_subs; dd++) {
              ...
              if (d == (i+1)%tot_subs) { /* look-ahead, this is executed first (i.e., dd=0)  */
                  ...
                  magma_dtranspose(panel_local[d], panel_local_offset[d], ldpan[d], 
              ...
              if (d == (i+1)%tot_subs) {
                  ...
                  if (nb0 > 0) {
                      ...
                      if (i+1 < s) {
                          magma_dtranspose(  d_lAP[d%num_gpus], dlAP_offset + ((i+1)%(2+num_gpus))*nb*maxm, ldda, 
                                             inAT(d,loff,i_local), lddat, nb0, ldda, queues[2*(d%num_gpus)+1] );
                      } else {
                          magma_dtranspose2( d_lAP[d%num_gpus], dlAP_offset + ((i+1)%(2+num_gpus))*nb*maxm, ldda, 
                                             inAT(d,loff,i_local), lddat, nb0, ldda, queues[2*(d%num_gpus)+1] );
                      }
      ...
      if (nb0 > 0) {
          ...
          for (d=0; d<num_gpus; d++) {
              magma_dsetmatrix_async( rows, nb0, W(s), 0, ldw,
          ...
          for (d=0; d<tot_subs; d++) {
              if (d == 0) {
                  magma_dpermute_long2( lddat, inAT(d,0,0), lddat, ipiv, nb0, s*nb, queues[2*(d%num_gpus)] );
              } else {
                  magma_dpermute_long3( lddat, inAT(d,0,0), lddat, ipiv, nb0, s*nb, queues[2*(d%num_gpus)] );
              }
          }
          ...
          for (dd=0; dd<tot_subs; dd++) {
              ...
              if (d%num_gpus == id%num_gpus) {
                  ...
                  if (dd < num_gpus) {
                      magma_dtranspose2( panel_local[d], panel_local_offset[d], lddat, 
                  ...
                  if (d < num_gpus) {
                      ...
                      magma_dtranspose2( panel_local[d], panel_local_offset[d], nb, 
                                         d_lAP[d%num_gpus], dlAP_offset+(s%(2+num_gpus))*nb*maxm, cols, 
                                         rows, nb0, queues[2*(d%num_gpus)+1]);
                  }
                  if (d < id) i_local2++;
