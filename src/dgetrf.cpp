    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            ldda = lddat = maxdim;
            magma_dsetmatrix( m, n, a, 0, lda, dA, dA_offset, ldda, queue[0] );
            ...
            dAT_offset = 0;   
            magma_dtranspose2( dAT, dAT_offset, lddat, dA, dA_offset, ldda, m, n, queue[0] );
        }
        ...
        for( i = 0; i < s; i++ )
        {
            ...
            if (i>0){
                // download i-th panel 
                magma_dtranspose( dAP, 0, cols, dAT(i,i), lddat, nb, cols, queue[0] );

                magma_queue_sync(queue[0]);
                magma_dgetmatrix_async( m-i*nb, nb, dAP, 0, cols, work, 0, lda, 
                                        queue[1], NULL);
            ...
            magma_dpermute_long2( n, dAT, dAT_offset, lddat, ipiv, nb, i*nb, queue[0] );

            // upload i-th panel
            magma_dsetmatrix_async( m-i*nb, nb, work, 0, lda, dAP, 0, maxm,
                                    queue[1], NULL);
            magma_queue_sync( queue[1] );

            magma_dtranspose( dAT(i,i), lddat, dAP, 0, maxm, cols, nb, queue[0]);
        ...
        if ( nb0 > 0 ) {
            ...
            magma_dtranspose2( dAP, 0, maxm, dAT(s,s), lddat, nb0, rows, queue[0]);
            magma_queue_sync(queue[0]);
            magma_dgetmatrix_async( rows, nb0, dAP, 0, maxm, work, 0, lda, queue[1], NULL );
            magma_queue_sync(queue[1]);
            ...
            if (*info == 0 && iinfo > 0)
                *info = iinfo + s*nb;
            magma_dpermute_long2( n, dAT, dAT_offset, lddat, ipiv, nb0, s*nb, queue[0] );
    
            magma_dsetmatrix_async( rows, nb0, work, 0, lda, dAP, 0, maxm, queue[1], NULL );
            magma_queue_sync(queue[1]);
            magma_dtranspose2( dAT(s,s), lddat, dAP, 0, maxm, rows, nb0, queue[0]);
