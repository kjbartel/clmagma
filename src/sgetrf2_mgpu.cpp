/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zgetrf2_mgpu.cpp normal z -> s, Sat Nov 15 00:21:37 2014
    ...
    if( num_gpus > ceil((float)n/nb) ) {
      *info = -1;
      return *info;
        ...
        nb0 = min(mindim, nb);
        magmablas_stranspose( nb0, maxm, d_lAT(0,0,0), lddat, d_lAP[0], 0, maxm, queues[2*0+1] );
        ...
        for( j=0; j < s; j++ )
        {
            ...
            d = (j+1)%num_gpus;
            for( dd=0; dd<num_gpus; dd++ ) {
                magmablas_slaswp( lddat, d_lAT(d,0,0), lddat, j*nb + 1, j*nb + nb, ipiv, 1, queues[2*d] );
                d = (d+1)%num_gpus;
            }
            ...
            for( dd=0; dd<num_gpus; dd++ ) {
               ...
               */
                ...
                //magmablas_stranspose( cols, nb, d_lAP[d], 0, cols, panel_local[d], panel_local_offset[d], ldpan[d], queues[2*d] );
                
                /* gpu updating the trailing matrix */
                if(d == (j+1)%num_gpus){
                magma_queue_sync(queues[2*d]);
                magmablas_stranspose( cols, nb, d_lAP[d], 0, cols, panel_local[d], panel_local_offset[d], ldpan[d], queues[2*d+1] );
                magma_queue_sync(queues[2*d+1]);
                ...
                if( d == (j+1)%num_gpus ) 
                {
                    ...
                    if( nb0 > 0 ) {
                        ...
                        magmablas_stranspose(   nb0, ldda, d_lAT(d,loff,i_local), lddat, d_lAP[d], 0, ldda, queues[2*d+1] );
        ...
        if( nb0 > 0 ) {
            ...
            for( d=0; d<num_gpus; d++ ) {
                magmablas_slaswp( lddat, d_lAT(d,0,0), lddat, s*nb + 1, s*nb + nb0, ipiv, 1, queues[2*d] );
            }
            ...
            for( d=0; d<num_gpus; d++ ) {
                ...
                if( d == id ) {
                    ...
                    magmablas_stranspose( rows, nb0, d_lAP[d], 0, cols, panel_local[d], panel_local_offset[d], lddat, queues[2*d+1] );
                    ...
                    magmablas_stranspose( rows, nb0, d_lAP[d], 0, cols, panel_local[d], panel_local_offset[d], nb, queues[2*d+1] );
                    //cublasStrsm
                    magma_strsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit, 
                                 nb1, nb0, c_one,
