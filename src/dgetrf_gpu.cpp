/*
    -- clMAGMA (version 0.2.0) --
       ...
       @generated d Thu May 24 17:09:43 2012
...
#define inAT(i,j) dAT, dAT_offset + (i)*nb*lddat + (j)*nb
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) 
      {
        // use CPU code
        err = magma_malloc_host( (void**) &work, m*n*sizeof(double) );
    ...
    else 
      {
        ...
        if ( MAGMA_SUCCESS != magma_malloc( &dAP, nb*maxm*sizeof(double))) {
        ...
        if ((m == n) && (m % 32 == 0) && (ldda%32 == 0))
          {
            ...
            magma_dinplace_transpose( dAT, dAT_offset, ldda, lddat, queue );
          }
        else 
          {
            dAT_offset = 0;
            if ( MAGMA_SUCCESS != magma_malloc( &dAT, maxm*maxn*sizeof(double))) {
              magma_free( dAP );
              *info = MAGMA_ERR_DEVICE_ALLOC;
              return *info;
            }
        ...
        if ( MAGMA_SUCCESS != magma_malloc_host((void**)&work, maxm*nb*sizeof(double)) ) {
          magma_free( dAP );
          if (! ((m == n) && (m % 32 == 0) && (ldda%32 == 0)) )
            magma_free( dAT );
        ...
        for( i=0; i<s; i++ )
            {
                ...
                magma_dpermute_long2( dAT, dAT_offset, lddat, ipiv, nb, i*nb, queue );
        ...
        if ( (*info == 0) && (iinfo > 0) )
            *info = iinfo + s*nb;
        magma_dpermute_long2( dAT, dAT_offset, lddat, ipiv, nb0, s*nb, queue );
        ...
        if ((m == n) && (m % 32 == 0) && (ldda%32 == 0)) {
          magma_dinplace_transpose( dAT, dAT_offset, lddat, ldda, queue );
        }
        else {
