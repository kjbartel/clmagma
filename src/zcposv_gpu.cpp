    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magmablas_zlanhe(MagmaInfNorm, uplo, n, dA(0,0), ldda, 
     ...
     */
    magmablas_zlag2c( n, nrhs, dB(0,0), lddb, dSX(0,0), lddsx, queue, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magmablas_zlat2c( uplo, n, dA(0,0), ldda, dSA(0,0), lddsa, queue, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_cpotrf_gpu( uplo, n, dSA(0,0), lddsa, queue, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_cpotrs_gpu( uplo, n, nrhs, dSA(0,0), lddsa, dSX(0,0), lddsx, queue, info );

    // residual dR = dB - dA*dX in double precision
    magmablas_clag2z( n, nrhs, dSX(0,0), lddsx, dX(0,0), lddx, queue, info );
    magmablas_zlacpy( MagmaUpperLower, n, nrhs, dB(0,0), lddb, dR(0,0), lddr, queue );
    if ( nrhs == 1 ) {
        magma_zhemv( uplo, n,
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magmablas_zlag2c( n, nrhs, dR(0,0), lddr, dSX(0,0), lddsx, queue, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
...
FALLBACK:
    ...
    magma_zpotrf_gpu( uplo, n, dA(0,0), ldda, queue, info );
    if (*info == 0) {
        magmablas_zlacpy( MagmaUpperLower, n, nrhs, dB(0,0), lddb, dX(0,0), lddx, queue);
        magma_zpotrs_gpu( uplo, n, nrhs, dA(0,0), ldda, dX(0,0), lddx, queue, info );
    }
