/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated from zpotrf_msub.cpp normal z -> s, Sat Nov 15 00:21:37 2014
...
/*  -- clMAGMA (version 1.3.0) --
    ...
    *info = 0;
    ...
    if (num_gpus == 1 && ((nb <= 1) || (nb >= n)) ) {
        ...
        #ifdef USE_PINNED_CLMEMORY
        cl_mem buffer = clCreateBuffer(gContext, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(float)*n*nb*h, NULL, NULL);
        for (d=0; d<num_gpus; d++) {
            work = (float*)clEnqueueMapBuffer(queues[2*d], buffer, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, 
                                                           sizeof(float)*n*nb*h, 0, NULL, NULL, NULL);
        }
        #else
        ...
        if (uplo == MagmaUpper) {
            ...
            magma_spotrf2_msub(num_subs, num_gpus, uplo, n, n, 0, 0, nb, d_lA, 0, ldda, 
                               dwork, lddp, work, n, h, queues, info);
            //magma_spotrf3_msub(num_subs, num_gpus, uplo, n, n, 0, 0, nb, d_lA, 0, ldda, 
            //                   dwork, lddp, work, n, h, queues, info);
            /* with three streams */
            //magma_spotrf3_msub(num_gpus, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, n,  
            //                   h, stream, event, info);
        } else {
            /* with two queues for each device */
            magma_spotrf2_msub(num_subs, num_gpus, uplo, n, n, 0, 0, nb, d_lA, 0, ldda, 
            ...
            //magma_spotrf3_msub(num_subs, num_gpus, uplo, n, n, 0, 0, nb, d_lA, 0, ldda, 
            //                   dwork, lddp, work, nb*h, h, queues, info);
            //magma_spotrf4_msub(num_subs, num_gpus, uplo, n, n, 0, 0, nb, d_lA, 0, ldda, 
            //                   dwork, lddp, work, nb*h, h, queues, info);
            /* with three streams */
            //magma_spotrf3_msub(num_gpus, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, nb*h, 
            //                   h, stream, event, info);
        }
