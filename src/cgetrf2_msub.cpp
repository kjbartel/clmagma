/*
    -- clMAGMA (version 1.3.0) --
       ...
       @generated from zgetrf2_msub.cpp normal z -> c, Sat Nov 15 00:21:37 2014
...
#define W(j)     (w +((j)%(1+ngpu))*nb*ldw)
    ...
    magmaFloatComplex_ptr dpanel_local[MagmaMaxSubs * MagmaMaxGPUs];
    ...
    *info = 0;
    ...
    if (tot_subs > ceil((float)n/nb)) {
        *info = -1;
        ...
        nb0 = min(mindim, nb);
        magmablas_ctranspose(  nb0, maxm, d_lAT(0,0,0), lddat, d_lAP[0], dlAP_offset, maxm, queues[2*0+1] );
        ...
        for (j=0; j < s; j++) {
            ...
            for (dd=0; dd < tot_subs; dd++) {
                magmablas_claswp( lddat, d_lAT(d,0,0), lddat, j*nb + 1, j*nb + nb, ipiv, 1, queues[2*(d%ngpu)] );
                d = (d+1)%tot_subs;
            }
        ...
        if (nb0 > 0) {
            ...
            for (d=0; d < ngpu; d++) {
                magma_csetmatrix_async( rows, nb0, W(s), ldw,
            ...
            for (d=0; d < tot_subs; d++) {
                magmablas_claswp( lddat, d_lAT(d,0,0), lddat, s*nb + 1, s*nb + nb0, ipiv, 1, queues[2*(d%ngpu)] );
            }
