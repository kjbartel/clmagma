    ...
    if (nb <= 1 || nb >= min(m,n)) 
      {
        ...
        chk( magma_zgetmatrix( m, n, dA, dA_offset, ldda, work, 0, m, queues[0] ));
        lapackf77_zgetrf(&m, &n, work, &m, ipiv, info);
        chk( magma_zsetmatrix( m, n, work, 0, m, dA, dA_offset, ldda, queues[0] ));
    ...
    else 
      {
        ...
        if ((m == n) && (m % 32 == 0) && (ldda%32 == 0))
        ...
        else 
          {
            ...
            magma_ztranspose2( dAT, dAT_offset, lddat, dA, dA_offset,  ldda, m, n, queues[0] );
        }
        ...
        */
        cl_mem buffer = clCreateBuffer(gContext, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(magmaDoubleComplex)*nb*maxm, NULL, NULL);
        work = (magmaDoubleComplex*)clEnqueueMapBuffer(queues[0], buffer, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, nb*maxm*sizeof(magmaDoubleComplex), 0, NULL, NULL, NULL);

        for( i=0; i<s; i++ )
            {
                // download i-th panel
                cols = maxm - i*nb;
                magma_ztranspose( dAP, 0, cols, inAT(i,i), lddat, nb, cols, queues[0] );
                clFlush(queues[0]);
                magma_queue_sync(queues[0]);
                magma_zgetmatrix_async(m-i*nb, nb, dAP, 0, cols, work, 0, lddwork, queues[1], NULL);
                clFlush(queues[1]);
                if ( i>0 ){
                ...
                magma_zpermute_long2(n, dAT, dAT_offset, lddat, ipiv, nb, i*nb, queues[0] );
                clFlush(queues[0]);

                // upload i-th panel
                magma_zsetmatrix_async(m-i*nb, nb, work, 0, lddwork, dAP, 0, maxm, queues[1], NULL);
                magma_queue_sync(queues[1]);
                magma_ztranspose(inAT(i,i), lddat, dAP, 0, maxm, cols, nb, queues[0]);
                clFlush(queues[0]);
        ...
        magma_ztranspose2( dAP, 0, maxm, inAT(s,s), lddat, nb0, rows, queues[0] );
        clFlush(queues[0]);
        magma_queue_sync(queues[0]);
        magma_zgetmatrix_async(rows, nb0, dAP, 0, maxm, work, 0, lddwork, queues[1], NULL);
        magma_queue_sync(queues[1]);
        // do the cpu part
        lapackf77_zgetrf( &rows, &nb0, work, &lddwork, ipiv+s*nb, &iinfo);
        if ( (*info == 0) && (iinfo > 0) )
            *info = iinfo + s*nb;
        magma_zpermute_long2(n, dAT, dAT_offset, lddat, ipiv, nb0, s*nb, queues[0] );
        clFlush(queues[0]);
        // upload i-th panel
        magma_zsetmatrix_async(rows, nb0, work, 0, lddwork, dAP, 0, maxm, queues[1], NULL);
        magma_queue_sync(queues[1]);
        magma_ztranspose2( inAT(s,s), lddat, dAP, 0, maxm, rows, nb0, queues[0] );
        clFlush(queues[0]);
