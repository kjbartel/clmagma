    ...
    magma_err_t*   info )
{
    ...
    *info = 0;
    ...
    int num = 1;
    chk( magma_get_devices( &device, 1, &num ));
    
    magma_queue_t queue[2];
    chk( magma_queue_create( device, &queue[0] ));
    chk( magma_queue_create( device, &queue[1] ));
    ...
    if ((nb <= 1) || (nb >= n)) {
        ...
        if ( uplo == MagmaUpper ) {
            ...
            for( j = 0; j < n; j += nb ) {
                ...
                chk( magma_zsetmatrix( jb, jb, work, 0, jb, dA(j,j), ldda, queue[1] ));
                ...
                if ( j+jb < n ) {
                    chk( magma_ztrsm(
                        ...
                        jb, n-j-jb,
                        z_one, dA(j, j),    ldda,
                               dA(j, j+jb), ldda, queue[0] ));
                    chk( magma_queue_sync( queue[0] ));
                }
            }
        }
        else {
            ...
            for( j = 0; j < n; j += nb ) {
                ...
                if ( j+jb < n ) {
                    chk( magma_ztrsm(
                        ...
                        n-j-jb, jb,
                        z_one, dA(j, j   ), ldda,
                               dA(j+jb, j), ldda, queue[0] ));
                    chk( magma_queue_sync( queue[1] ));
                }
            }
        }
    }
    
    chk( magma_queue_destroy( queue[0] ));
    chk( magma_queue_destroy( queue[1] ));
