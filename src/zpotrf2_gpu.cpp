...
*/
    ...
    if ((nb <= 1) || (nb >= n)) {
        // use unblocked code
        magma_zgetmatrix( n, n, dA, dA_offset, ldda, work, n, queue[1] );
        lapackf77_zpotrf( lapack_uplo_const(uplo), &n, work, &n, info );
        magma_zsetmatrix( n, n, work, n, dA, dA_offset, ldda, queue[1] );
    }
    else {
        if ( uplo == MagmaUpper ) {
            // --------------------
            ...
            for( j = 0; j < n; j += nb ) {
                ...
                magma_zgetmatrix_async( jb, jb, dA(j,j), ldda, work, jb, queue[0], NULL );
                ...
                //clFinish(queue[0]);
                magma_zsetmatrix_async( jb, jb, work, jb, dA(j,j), ldda, queue[0], NULL );
                magma_queue_sync(queue[0]);
